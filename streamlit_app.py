    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # Logic to identify the correct file to read
                target_file = None
                
                if format_choice in ["SRT", "Clean TXT"]:
                    # We strictly look for the .srt extension generated by FFmpeg
                    for f in files:
                        if f.endswith('.srt'):
                            target_file = f
                            break
                else:
                    # 'Raw (VTT)' selected: Look for the original .vtt source
                    for f in files:
                        if f.endswith('.vtt'):
                            target_file = f
                            break
                    # If no VTT but other formats exist, take the first subtitle-like file
                    if not target_file:
                        for f in files:
                ...
                if format_choice == "Clean TXT":
                    # Convert content to a clean transcript
                    ...
                    # Ensure the filename extension is .srt
                    final_name = f"{sanitize_filename(video_title)}.srt"
                    return content.encode('utf-8'), final_name
                else:
                    # Raw (VTT) or original source extension
...
def render_download_options(info, url, cookies_path):
    ...
    with col_fmt:
        format_choice = st.radio(
            ...
            horizontal=True,
            help="SRT: Standard subtitle file. Raw: Original VTT source. Clean TXT: Text-only transcript."
        )
    ...
    if st.button("🚀 Generate Download Link"):
        with st.spinner("Generating file..."):
            ...
            if data:
                ...
                st.error("Extraction failed. Please try a different language or format.")
